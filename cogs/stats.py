...
# ── /mmr-history ──────────────────────────────────────────────────────────
    ...
    @app_commands.command(
        name="mmr-history",
        description="Show RR gain/loss over the last competitive matches."
    )
    @app_commands.describe(
        member="Discord user (defaults to you)",
        count="Number of games to show (default: 10, max: 15)",
    )
    async def mmr_history(
        ...
        try:
            count = max(1, min(count, 15))  # clamp 1–15
            ...
            if data.get("status") != 200:
                ...
                await interaction.followup.send(
                    f"❌ Could not fetch MMR history for `{name}#{tag}` — {err}"
                )
                return
            ...
            if not history:
                await interaction.followup.send(
                    f"No MMR history found for `{name}#{tag}`."
                )
                return
            ...
            # Current rank from first entry (most recent)
            latest      = history[0]
            ...
            # Embed colour: green if net positive, red if net negative
            ...
            for entry in history:
                ...
                # Shorten date — "Tuesday, January 11, 2022 9:52 PM" → "Jan 11"
                try:
    ...
    async def stats(self, interaction: discord.Interaction, member: Optional[discord.Member] = None):
        ...
        try:
            ...
            if mmr_data.get("status") == 200:
                ...
                # Use the actual rank badge image from the API
            ...
            embed.set_footer(text="valorant-api.henrikdev.xyz  ·  /link to update")
            await interaction.followup.send(embed=embed)
    ...
    async def match(
        ...
        try:
            ...
            for m in matches:
                ...
                stats  = me.get("stats", {})
                k, d, a = stats.get("kills", 0), stats.get("deaths", 1), stats.get("assists", 0)
